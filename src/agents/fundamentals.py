...
from src.tools.api import get_financial_metrics, get_financial_metrics_async
...
PERSONA_BACKSTORY = "Aurora spent years running deep fundamental screens and now synthesises profitability, growth, health, and valuation signals before advising the PM."
...
async def fundamentals_analyst_agent_async(state: AgentState, agent_id: str = "fundamentals_analyst_agent"):
    """Async fundamentals analyst using persona observations."""
    data = state["data"]
    end_date = data["end_date"]
    tickers = data["tickers"]
    api_key = get_api_key_from_state(state, "FINANCIAL_DATASETS_API_KEY")
    ...
    for ticker in tickers:
        ...
        financial_metrics = await get_financial_metrics_async(
            ticker=ticker,
            end_date=end_date,
            period="ttm",
            limit=10,
    ...
    await update_analyst_signals_async(state, agent_id, fundamental_analysis)
    progress.update_status(agent_id, None, "Done")
